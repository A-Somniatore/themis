...
type Mutation {
  ...
  """
  createUser(input: CreateUserInput!): CreateUserPayload! @operation(
  ...
  """
  updateUser(id: ID!, input: UpdateUserInput!): UpdateUserPayload! @operation(
  ...
  """
  deleteUser(id: ID!): DeleteUserPayload! @operation(
...
type Subscription {
  ...
  """
  userUpdated(userId: ID!): User! @operation(
...
type User {
  ...
  posts(first: Int = 10): PostConnection! @operation(
...
import { Resolvers, User, CreateUserInput, CreateUserPayload } from './generated/types';

// Implement this interface
export interface UserResolvers {
  Query: {
    user: (parent: unknown, args: { id: string }, ctx: Context) => Promise<User | null>;
    users: (parent: unknown, args: UsersArgs, ctx: Context) => Promise<UserConnection>;
  };
  Mutation: {
    createUser: (parent: unknown, args: { input: CreateUserInput }, ctx: Context) => Promise<CreateUserPayload>;
  };
}
```
...
import { GetUserRequest, GetUserResponse, CreateUserRequest, CreateUserResponse } from './types';
...
export function createUsersServiceRouter(handlers: UsersServiceHandlers): Router {
  const router = Router();
...
- [ ] Implement `themis-core` (contract model)
- [ ] Implement `themis-openapi` (OpenAPI parsing)
- [ ] Basic CLI scaffold
