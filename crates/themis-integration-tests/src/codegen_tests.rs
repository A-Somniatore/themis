...
fn test_typescript_codegen_produces_valid_code() {
    ...
    assert!(code.contains("export") || code.contains("interface") || code.contains("type"), 
        "Should have exported types");
    
    // Check for proper typing
    assert!(code.contains("string") || code.contains("number") || code.contains("boolean"),
...
fn test_python_codegen_produces_valid_code() {
    ...
    assert!(code.contains("dataclass") || code.contains("class "), 
        "Should have class definitions");
    
    // Check for type hints
    assert!(code.contains("str") || code.contains("int") || code.contains("Optional"),
...
fn test_all_languages_generate_code() {
    let contract = parse_openapi(SECURE_CONTRACT)
        .expect("Should parse");
...
fn test_codegen_with_docs() {
    ...
    assert!(ts_code.contains("/**") || ts_code.contains("//") || ts_code.contains("*"),
...
fn test_codegen_without_docs() {
    ...
    assert!(rust_code.contains("struct") || rust_code.contains("enum") || !rust_output.files.is_empty(),
...
"#;
    ...
    let contract = parse_openapi(contract_with_optional)
        .expect("Should parse");
    ...
    let rust_output = rust_gen.generate(&contract)
        .expect("Should generate Rust");
    let rust_code = all_code(&rust_output);
    assert!(rust_code.contains("Option<") || rust_code.contains("option") || rust_code.contains("None"),
...
"#;
    ...
    let contract = parse_openapi(contract_with_arrays)
        .expect("Should parse");
    ...
    let rust_output = rust_gen.generate(&contract)
        .expect("Should generate Rust");
    let rust_code = all_code(&rust_output);
    assert!(rust_code.contains("Vec<") || rust_code.contains("vec") || rust_code.contains("["),
    ...
    let ts_output = ts_gen.generate(&contract)
        .expect("Should generate TypeScript");
    let ts_code = all_code(&ts_output);
    assert!(ts_code.contains("[]") || ts_code.contains("Array") || ts_code.contains("array"),
    ...
    let py_output = py_gen.generate(&contract)
        .expect("Should generate Python");
    let py_code = all_code(&py_output);
    assert!(py_code.contains("list") || py_code.contains("List") || py_code.contains("["),
...
"#;
    ...
    let contract = parse_openapi(contract_with_enum)
        .expect("Should parse");
    ...
    let rust_output = rust_gen.generate(&contract)
        .expect("Should generate Rust");
    let rust_code = all_code(&rust_output);
    assert!(rust_code.contains("enum") || rust_code.contains("Active") || rust_code.contains("active") || rust_code.contains("Status"),
    ...
    let ts_output = ts_gen.generate(&contract)
        .expect("Should generate TypeScript");
    let ts_code = all_code(&ts_output);
    assert!(ts_code.contains("active") || ts_code.contains("Status") || ts_code.contains("enum"),
...
fn test_generator_config_options() {
    ...
    let output = gen.generate(&contract)
        .expect("Should generate");
    let code = all_code(&output);

    // Check that code was generated
    assert!(!code.is_empty(), "Should generate code with validation enabled");
