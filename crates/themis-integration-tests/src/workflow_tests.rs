...
fn test_full_workflow_happy_path() {
    ...
    let contract = parse_openapi(USERS_SERVICE_V1).expect("Should parse users-service v1");
    ...
    assert!(
        rust_code.contains("struct") || rust_code.contains("enum"),
    ...
    println!(
        "  - Parsed contract with {} operations",
...
fn test_version_comparison_workflow() {
    // Parse both versions
    let v1 = parse_openapi(USERS_SERVICE_V1).expect("Should parse v1");
    let v2 = parse_openapi(USERS_SERVICE_V2).expect("Should parse v2");
...
fn test_minimal_contract_workflow() {
    ...
    let rust_code = all_code(&rust_output);
    assert!(
        rust_code.contains("GetHealth")
            || rust_code.contains("get_health")
            || rust_code.contains("Health"),
...
fn test_breaking_change_detection() {
    ...
    let v1 = parse_openapi(MINIMAL_CONTRACT).expect("Should parse v1");
    let v2 = parse_openapi(BREAKING_CHANGE_CONTRACT).expect("Should parse v2");
...
fn test_artifact_round_trip() {
    ...
    // Serialize to JSON
    let json = artifact.to_json().expect("Should serialize to JSON");
...
fn test_multi_language_codegen() {
    ...
    assert!(!rust_code.is_empty(), "Rust code should not be empty");
    assert!(
        rust_code.contains("serde") || rust_code.contains("Serialize"),
        "Rust code should use serde"
    );
    assert!(
        rust_code.contains("pub struct"),
        "Rust code should have public structs"
    );
    ...
    assert!(
        ts_code.contains("export") || ts_code.contains("interface") || ts_code.contains("type"),
    ...
    assert!(
        py_code.contains("dataclass") || py_code.contains("class "),
...
fn test_lint_configuration_affects_results() {
    ...
    println!(
        "Relaxed: {} issues ({} errors, {} warnings)",
